...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train 5 ExtraTrees models from a shared global-65 subset with local random 80% subsets.")
    parser.add_argument("--data-dir", default="src/odor_competition/data")
    parser.add_argument("--output-dir", default="artifacts_extratrees_corr_optuna/12_et5_global65_local80")
    parser.add_argument("--submission-prefix", default="et5_global65_local80")
...
def _sample_local_subset(columns: list[str], *, seed: int) -> list[str]:
    if not columns:
        return []

    rng = np.random.default_rng(seed)
    desired = int(round(len(columns) * LOCAL_SUBSET_FRACTION))
...
def fit_feature_preprocessor(
    ...
    *,
    ...
    local_subset_columns = _sample_local_subset(global_selected, seed=local_subset_seed)
...
def main() -> None:
        ...
        model_reports.append(
            {
                ...
                "feature_strategy": "shared_global_65_selection_then_local_random_80_subset",
    ...
    summary = {
        ...
        "model": "5x ExtraTrees on shared global-65 subset + local random-80% subsets + Dirichlet target-wise blend + calibration",
        ...
        "notes": {
            ...
            "ensemble_diversity": "All 5 ExtraTrees share the same globally selected 65 features, then each model uses a different random 80% local subset, plus a different seed and hyperparameter ranges.",
        },
        "preprocessing": {
            ...
            "local_selection": {
                "subset_fraction": float(LOCAL_SUBSET_FRACTION),
            },
            "feature_selection": "full pool -> global signal filter -> global correlation pruning -> keep 65 -> local random 80% subset per model",
