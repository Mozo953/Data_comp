...
DEFAULT_PARAMS_JSON = "artifacts_extratrees_corr_optuna/02_experiments_OPEN/q20_feat45_corr990_cv6_trials24/best_score_actuel.json"
DEFAULT_OUTPUT_DIR = "artifacts_extratrees_corr_optuna/20_experiments_best_et_optuna5"
...
class FeaturePreprocessor:
    ...
    def transform(self, features: pd.DataFrame, *, eps: float) -> pd.DataFrame:
        expanded = build_log_ratio_features(features, eps=eps)
        return expanded[self.selected_columns].copy()


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Retrain the best ExtraTrees model with a small Optuna search around the current best params."
    )
    parser.add_argument("--data-dir", default="src/odor_competition/data", help="Directory with X_train.csv/X_test.csv/y_train.csv.")
    parser.add_argument("--params-json", default=DEFAULT_PARAMS_JSON, help="JSON file containing the best ET parameters.")
    parser.add_argument("--output-dir", default=DEFAULT_OUTPUT_DIR, help="Directory to save artifacts.")
    parser.add_argument("--submission-prefix", default="best_et_optuna5_relaxed", help="Submission filename prefix.")
    parser.add_argument("--cv-folds", type=int, default=6, help="Cross-validation fold count.")
    parser.add_argument("--optuna-trials", type=int, default=5, help="Optuna trials for the relaxed search.")
    parser.add_argument("--optuna-holdout", type=float, default=0.2, help="Validation fraction for Optuna objective.")
    parser.add_argument("--search-flex", type=float, default=0.05, help="Relative search window around the base params.")
    parser.add_argument("--corr-threshold", type=float, default=0.99, help="Feature correlation threshold for pruning.")
    parser.add_argument("--signal-quantile", type=float, default=0.2, help="Keep only features above this signal quantile before pruning.")
    parser.add_argument("--max-selected-features", type=int, default=45, help="Maximum number of features kept after pruning.")
    ...
    parser.add_argument("--max-tail-ratio", type=float, default=250.0, help="Drop unstable features with extreme tail ratio above this value.")
    parser.add_argument("--random-state", type=int, default=42, help="Seed for splits and model.")
    parser.add_argument("--skip-optuna", action="store_true", help="Skip Optuna and use the base params from the JSON file.")
    ...
    if args.search_flex <= 0.0:
        raise ValueError("--search-flex must be > 0.")
    return args
...
def _bounded_int(base_value: int, flex: float, min_value: int = 1) -> tuple[int, int]:
    lower = max(min_value, int(np.floor(base_value * (1.0 - flex))))
    upper = max(lower, int(np.ceil(base_value * (1.0 + flex))))
    if lower == upper:
        upper = lower + 1
    return lower, upper


def _bounded_float(base_value: float, flex: float, *, lower_cap: float | None = None, upper_cap: float | None = None) -> tuple[float, float]:
    lower = base_value * (1.0 - flex)
    upper = base_value * (1.0 + flex)
    if lower_cap is not None:
        lower = max(lower_cap, lower)
    if upper_cap is not None:
        upper = min(upper_cap, upper)
    if lower >= upper:
        upper = lower + max(1e-3, abs(base_value) * 1e-3)
    return lower, upper


def _build_relaxed_search_space(base_params: dict, flex: float) -> dict:
    search_space: dict = {}
    for key in ["n_estimators", "max_depth", "min_samples_split", "min_samples_leaf"]:
        lower, upper = _bounded_int(int(base_params[key]), flex)
        search_space[key] = (lower, upper)
    lower, upper = _bounded_float(float(base_params["max_features"]), flex, lower_cap=0.05, upper_cap=1.0)
    search_space["max_features"] = (lower, upper)
    if bool(base_params.get("bootstrap", True)):
        lower, upper = _bounded_float(float(base_params.get("max_samples", 1.0)), flex, lower_cap=0.1, upper_cap=1.0)
        search_space["max_samples"] = (lower, upper)
    return search_space
...
def optimize_params(
    ...
    *,
    ...
    if not OPTUNA_AVAILABLE:
        print("Optuna non disponible, utilisation des parametres de base.")
    ...
    X_fit_pre = fit_pre.transform(X_fit, eps=ratio_eps)
    X_valid_pre = fit_pre.transform(X_valid, eps=ratio_eps)

    search_space = _build_relaxed_search_space(base_params, search_flex)
    ...
    def objective(trial: optuna.Trial) -> float:
        params = {
            ...
            "max_features": trial.suggest_float("max_features", search_space["max_features"][0], search_space["max_features"][1]),
            "bootstrap": bootstrap,
            "random_state": random_state,
            "n_jobs": -1,
        }
        if bootstrap and "max_samples" in search_space:
            params["max_samples"] = trial.suggest_float("max_samples", search_space["max_samples"][0], search_space["max_samples"][1])
...
def run_cv(
    ...
    *,
    ...
    for fold_idx, (fit_idx, valid_idx) in enumerate(kfold.split(X_train), start=1):
        ...
        X_fit_pre = pre.transform(X_fit, eps=ratio_eps)
        X_valid_pre = pre.transform(X_valid, eps=ratio_eps)
...
def fit_full_and_predict(
    ...
    *,
    ...
    X_train_pre = pre.transform(X_train, eps=ratio_eps)
    X_test_pre = pre.transform(X_test, eps=ratio_eps)
...
def main() -> None:
    ...
    base_params = load_best_params_from_json(params_json_path)
    base_params["random_state"] = args.random_state
    ...
    summary = {
        ...
        "experiment": "best_et_optuna5_relaxed",
        "data_dir": str(data_dir),
        "params_json": str(params_json_path.relative_to(ROOT)),
        "feature_pipeline": {
        ...
        "optuna": {
            ...
            "search_flex": float(args.search_flex),
