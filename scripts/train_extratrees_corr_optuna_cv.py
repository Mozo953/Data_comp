...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train ExtraTrees with correlation-aware preprocessing, ratio features, light Optuna, and CV."
    )
    parser.add_argument("--data-dir", default="src/odor_competition/data", help="Directory with X_train.csv/X_test.csv/y_train.csv.")
    parser.add_argument("--output-dir", default="artifacts_extratrees_corr_optuna", help="Directory to save artifacts.")
    parser.add_argument("--submission-prefix", default="extratrees_corr_optuna_cv", help="Submission filename prefix.")
    parser.add_argument("--corr-threshold", type=float, default=0.985, help="Feature correlation threshold for pruning.")
    parser.add_argument("--signal-quantile", type=float, default=0.35, help="Keep only features above this signal quantile before correlation pruning.")
    parser.add_argument("--max-selected-features", type=int, default=28, help="Maximum number of features kept after signal and correlation pruning.")
    parser.add_argument("--ratio-eps", type=float, default=1e-3, help="Stability epsilon for ratio denominators.")
    parser.add_argument("--cv-folds", type=int, default=3, help="Small CV fold count.")
    parser.add_argument("--optuna-trials", type=int, default=16, help="Light Optuna trials.")
    parser.add_argument("--optuna-holdout", type=float, default=0.2, help="Validation fraction for Optuna objective.")
    parser.add_argument("--plot-learning-curves", action="store_true", help="Generate a learning curves plot and JSON summary.")
    parser.add_argument("--learning-curve-sample-size", type=int, default=30000, help="Maximum number of training rows used for learning curves.")
    parser.add_argument("--learning-curve-points", type=int, default=5, help="Number of learning curve train sizes.")
    parser.add_argument("--learning-curve-cv", type=int, default=3, help="Cross-validation folds used for learning curves.")
    parser.add_argument("--params-json", default=None, help="Optional path to a previous summary JSON to reuse best_params.")
    parser.add_argument("--learning-curves-only", action="store_true", help="Only generate learning curves and skip full training/submission export.")
    ...
    if args.learning_curve_sample_size < 1:
        raise ValueError("--learning-curve-sample-size must be >= 1.")
    if args.learning_curve_points < 2:
        raise ValueError("--learning-curve-points must be >= 2.")
    if args.learning_curve_cv < 2:
        raise ValueError("--learning-curve-cv must be >= 2.")
...
def build_ratio_features(features: pd.DataFrame, *, eps: float = 1e-3) -> pd.DataFrame:
    ...
    for i, left in enumerate(columns):
        for right in columns[i + 1 :]:
            ...
            # Log-compress ratio tails to reduce split instability from extreme values.
...
def generate_learning_curves(
    ...
    *,
    ...
    sample = X_train.sample(n=sample_n, random_state=random_state)
    sample_index = sample.index
    X_sample = X_train.loc[sample_index]
    y_sample_model = y_train_model.loc[sample_index]
    y_sample_full = y_train_full.loc[sample_index]

    train_size_fractions = np.linspace(0.2, 1.0, train_size_points)
    kfold = KFold(n_splits=cv_folds, shuffle=True, random_state=random_state)

    learning_curve_rows: list[dict] = []

    for size_index, fraction in enumerate(train_size_fractions, start=1):
        fold_train_scores: list[float] = []
        fold_valid_scores: list[float] = []
        fold_train_size = max(50, int(round(len(X_sample) * float(fraction))))

        for fold_id, (fit_idx, valid_idx) in enumerate(kfold.split(X_sample), start=1):
            fit_pool = np.asarray(fit_idx)
            if fold_train_size >= len(fit_pool):
                sub_fit_idx = fit_pool
            else:
                local_seed = random_state + size_index * 1000 + fold_id
                local_rng = np.random.default_rng(local_seed)
                sub_fit_idx = local_rng.choice(fit_pool, size=fold_train_size, replace=False)

            X_fit = X_sample.iloc[sub_fit_idx]
            X_valid = X_sample.iloc[valid_idx]
            y_fit_model = y_sample_model.iloc[sub_fit_idx]
            y_valid_model = y_sample_model.iloc[valid_idx]
            y_fit_full = y_sample_full.iloc[sub_fit_idx]
            y_valid_full = y_sample_full.iloc[valid_idx]
            ...
            fit_pred_model = pd.DataFrame(
                model.predict(X_fit_pre),
                columns=y_fit_model.columns,
                index=X_fit_pre.index,
            )
            ...
            fold_train_scores.append(
                competition_rmse(y_fit_full, schema.expand_predictions(fit_pred_model))
            )
            fold_valid_scores.append(
                competition_rmse(y_valid_full, schema.expand_predictions(valid_pred_model))
            )

        learning_curve_rows.append(
            {
                "train_size_fraction": float(fraction),
                "train_size_rows": int(fold_train_size),
                "train_rmse_mean": float(np.mean(fold_train_scores)),
                "train_rmse_std": float(np.std(fold_train_scores)),
                "valid_rmse_mean": float(np.mean(fold_valid_scores)),
                "valid_rmse_std": float(np.std(fold_valid_scores)),
            }
        )
    ...
    x_values = learning_curve_df["train_size_rows"].to_numpy()
    train_mean = learning_curve_df["train_rmse_mean"].to_numpy()
    train_std = learning_curve_df["train_rmse_std"].to_numpy()
    valid_mean = learning_curve_df["valid_rmse_mean"].to_numpy()
    valid_std = learning_curve_df["valid_rmse_std"].to_numpy()

    ax.plot(x_values, train_mean, marker="o", linewidth=2.2, label="Train RMSE")
    ax.fill_between(x_values, train_mean - train_std, train_mean + train_std, alpha=0.18)
    ax.plot(x_values, valid_mean, marker="o", linewidth=2.2, label="Validation RMSE")
    ax.fill_between(x_values, valid_mean - valid_std, valid_mean + valid_std, alpha=0.18)
    ax.set_title("ExtraTrees learning curves")
    ax.set_xlabel("Training rows used")
    ax.set_ylabel("Competition RMSE")
    ax.grid(True, alpha=0.25)
    ax.legend()
    fig.tight_layout()

    plot_path = output_dir / f"{prefix}_{timestamp}_learning_curves.png"
    curve_json_path = output_dir / f"{prefix}_{timestamp}_learning_curves.json"
    fig.savefig(plot_path, dpi=160, bbox_inches="tight")
    plt.close(fig)

    learning_curve_payload = {
        "sample_size_used": int(sample_n),
        "train_size_points": int(train_size_points),
        "cv_folds": int(cv_folds),
        "plot_path": str(plot_path.relative_to(ROOT)),
        "curve_json_path": str(curve_json_path.relative_to(ROOT)),
        "rows": learning_curve_rows,
    }
    curve_json_path.write_text(json.dumps(learning_curve_payload, indent=2))
    return learning_curve_payload
    ...
    *,
    ...
    def objective(trial: optuna.Trial) -> float:
        if strong_anti_overfit:
            n_estimators = trial.suggest_int("n_estimators", 280, 500, step=20)
            ...
            max_features = trial.suggest_float("max_features", 0.35, 0.7)
...
def run_cv(
    ...
    *,
    ...
    for fold_idx, (fit_idx, valid_idx) in enumerate(kfold.split(X_train), start=1):
        ...
        y_valid_model = y_train_model.iloc[valid_idx]
        ...
        fold_reports.append(
            {
                ...
                "ratio_feature_count_generated": int(len(pre.ratio_columns)),
...
def main() -> None:
    ...
    # d15 is constant and duplicates are compressed through schema.model_targets.
    ...
    if args.learning_curves_only:
        learning_curves = generate_learning_curves(
            ...
            max_selected_features=args.max_selected_features,
            random_state=args.random_state,
            sample_size=args.learning_curve_sample_size,
            train_size_points=args.learning_curve_points,
            cv_folds=args.learning_curve_cv,
            output_dir=output_dir,
            prefix=args.submission_prefix,
            timestamp=timestamp,
        )
        print(json.dumps(learning_curves, indent=2))
        return
    ...
    summary = {
        ...
        "feature_pipeline": {
            ...
            "ratio_feature_count_generated": int(len(full_pre.ratio_columns)),
            "total_feature_count_before_pruning": int(X_train.shape[1] + len(full_pre.ratio_columns)),
