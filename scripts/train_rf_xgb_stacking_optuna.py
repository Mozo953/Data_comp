...
def generate_oof_predictions(
    ...
    Returns: (rf_oof, xgb_oof, meta_indices_for_sorting)
    """
...
def evaluate_feature_set(
    ...
    print(f"\n=== Optimizing feature mode: {feature_mode} ({X_train.shape[1]} features) ===")
    ...
    def callback(trial: optuna.Trial, study: optuna.Study) -> None:
        nonlocal last_heartbeat
        trial_count[0] += 1
        now = time.time()
        if now - last_heartbeat >= heartbeat_seconds or trial_count[0] <= 2:
            best_val = study.best_value
            elapsed = (now - t0) / 60.0
            trial_num = trial.number + 1
            print(f"  Trial {trial_num}/{n_trials}: rmse={trial.value:.6f}, best={best_val:.6f}, elapsed={elapsed:.1f}m")
            last_heartbeat = now
    ...
    print(f"  Best trial #{best_trial.number}: validation_rmse={best_val:.6f} (elapsed={elapsed:.1f}m)")
...
def main() -> None:
    ...
    print(f"\n=== Final Results ===")
    print(f"Total time: {elapsed_global:.1f}m")
    print(f"\nAll feature modes:")
    for res in results:
        print(f"  {res['feature_mode']:30s}: rmse={res['best_validation_rmse']:.6f} (trials={res['n_trials']})")

    print(f"\nBest overall: {overall_best['feature_mode']} with rmse={overall_best_score:.6f}")
