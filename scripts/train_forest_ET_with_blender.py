...
DEFAULT_MODEL_SPECS = [
    {
        "name": "et_subspace_1",
        "seed_offset": 0,
        "subspace_fraction": 0.70,
        "max_selected_features": 28,
        "signal_quantile": 0.35,
        "corr_threshold": 0.985,
        "bootstrap_choices": [False, True],
        "max_depth_min": 10,
        "max_depth_max": 14,
        "max_features_min": 0.28,
        "max_features_max": 0.52,
    ...
    },
    {
        ...
        "seed_offset": 17,
        "subspace_fraction": 0.70,
        "max_selected_features": 34,
        "signal_quantile": 0.30,
        "corr_threshold": 0.985,
        "bootstrap_choices": [False],
        "max_depth_min": 10,
        "max_depth_max": 16,
        "max_features_min": 0.35,
        "max_features_max": 0.62,
    ...
    },
    {
        ...
        "seed_offset": 34,
        "subspace_fraction": 0.70,
        "max_selected_features": 42,
        "signal_quantile": 0.25,
        "corr_threshold": 0.985,
        "bootstrap_choices": [True],
        "max_depth_min": 10,
        "max_depth_max": 14,
        "max_features_min": 0.45,
        "max_features_max": 0.78,
...
class FeaturePreprocessor:
    sampled_subspace_columns: list[str]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train 5 compact ExtraTrees models with Dirichlet blending.")
    parser.add_argument("--data-dir", default="src/odor_competition/data")
    parser.add_argument("--output-dir", default="artifacts_extratrees_corr_optuna/09_et5_dirichlet_blend")
    parser.add_argument("--submission-prefix", default="et5_dirichlet_blend")
...
def _sample_random_subspace(columns: list[str], spec: ModelSpec, *, seed: int) -> list[str]:
    if not columns:
        return []

    rng = np.random.default_rng(seed)
    desired = int(round(len(columns) * spec.subspace_fraction))
    desired = max(spec.max_selected_features * 2, desired)
...
def fit_feature_preprocessor(
    ...
    *,
    ...
    sampled_subspace_columns = _sample_random_subspace(list(expanded.columns), spec, seed=subspace_seed)
    sampled = expanded[sampled_subspace_columns].copy()

    signal = feature_target_signal(sampled, y_train_model)
    min_signal = float(signal.quantile(spec.signal_quantile))
    filtered_signal = signal[signal >= min_signal].sort_values(ascending=False)
    ordered_columns = list(filtered_signal.index)
    if not ordered_columns:
        ordered_columns = list(signal.sort_values(ascending=False).head(spec.max_selected_features).index)

    selected, dropped = _prune_correlated_by_order(sampled, ordered_columns, threshold=spec.corr_threshold)
    if len(selected) > spec.max_selected_features:
        selected = selected[: spec.max_selected_features]

    if not selected:
        fallback_count = min(spec.max_selected_features, len(sampled.columns))
        selected = list(signal.sort_values(ascending=False).head(fallback_count).index)

    return FeaturePreprocessor(
        sampled_subspace_columns=sampled_subspace_columns,
...
def suggest_et_params(trial: optuna.Trial, spec: ModelSpec, *, random_state: int, n_jobs: int) -> dict:
    ...
    params = {
        ...
        "max_depth": trial.suggest_int("max_depth", spec.max_depth_min, spec.max_depth_max, step=2),
        ...
        fold_reports.append(
            {
                ...
                "sampled_subspace_feature_count": int(len(preprocessor.sampled_subspace_columns)),
...
def evaluate_model_holdout(
    ...
    *,
    ...
    log_progress(
        ...
        f"from a sampled subspace of {len(preprocessor.sampled_subspace_columns)}/{preprocessor.global_feature_count}"
    )
    model = make_et_model(params)
    model.fit(X_fit_model, y_fit_model)
...
def fit_full_model_predict(
    ...
    *,
    ...
    preprocessor = fit_feature_preprocessor(
        ...
        spec,
        ratio_eps=ratio_eps,
        subspace_seed=params["random_state"],
    )
    ...
    meta = {
        "global_feature_count": int(preprocessor.global_feature_count),
        "sampled_subspace_feature_count": int(len(preprocessor.sampled_subspace_columns)),
        "sampled_subspace_columns": preprocessor.sampled_subspace_columns,
        "selected_feature_count": int(len(preprocessor.selected_columns)),
...
def optimize_dirichlet_blend_per_target(
    ...
    *,
    ...
    log_progress(
        f"Blend: optimizing target-wise Dirichlet weights with {sample_count} sampled simplex points"
    )
    ...
    candidates.append(np.eye(len(model_names), dtype=np.float32))
...
def fit_target_calibration(
    ...
    log_progress("Calibration: fitting per-target linear calibration and shrinkage")
    ...
    shrink_grid = [0.0, 0.02, 0.04, 0.06, 0.08]

    for target in blended_oof.columns:
        ...
        best_shrinkage = 0.0
        best_score = weighted_rmse_1d(y, linear)
        best_prediction = linear

        for shrinkage in shrink_grid[1:]:
            candidate = np.clip(((1.0 - shrinkage) * linear) + (shrinkage * y_mean), 0.0, 1.0)
            score = weighted_rmse_1d(y, candidate)
            if score < best_score:
                best_score = score
...
def main() -> None:
        ...
        model_reports.append(
            {
                ...
                "feature_strategy": "shared_global_pool_with_model_specific_random_subspace",
    ...
    calibration, calibrated_oof = fit_target_calibration(blended_oof, y_train_model)
    ...
    summary = {
        ...
        "model": "5x ExtraTrees on shared global feature pool + random subspaces + Dirichlet target-wise blend + calibration",
        ...
        "notes": {
            ...
            "ensemble_diversity": "Each of the 5 ExtraTrees models uses the same global feature pool but a different random feature subspace, different seed, and different hyperparameter ranges.",
        },
        "preprocessing": {
            ...
            "feature_selection": "random subspace per model -> signal filter -> correlation pruning",
